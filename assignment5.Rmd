...
predict_m <- predict(lm_gem1, newdata = ge_year, se.fit = TRUE, interval="confidence")
predict_f <- predict(lm_gef1, newdata = ge_year, se.fit = TRUE, interval="confidence")

ge_predm <- cbind(ge_year, predict_m)
ge_predf <- cbind(ge_year, predict_f)

# plot
ge_plot <- ggplot(ge_adddecyear, aes(x = year, y = total_females)) +
  geom_point() +
  geom_point(aes(x = year, y = total_males)) + 
  #geom_smooth(method = lm_gem1, se = T, size = 0.5, color = "gray20") +
  geom_ribbon(ge_predm, aes(ymin = )) +
  theme_classic()
